    ...
    return <section className="container mx-auto p-5 min-h-screen w-full flex items-center justify-center">
        ...
        <div className="max-w-120 w-full  border border-gray-200 rounded-lg flex flex-col px-6 py-8 items-center">
            ...
            <div className="flex items-center flex-col">
                ...
                <p className="mt-4 font-normal">Register your account to get started</p>
            </div>
            ...
            <form className="mt-6 w-full" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div>
                    <label htmlFor="email">
                        <span className="font-medium mb-1 block">Name</span>
                        <input className={`w-full outline-green-600 border ${errors?.name ? "border-red-500" : "border-gray-200"} rounded-md px-3 py-2`} type="text" name="name" {...register("name", { required: true })} placeholder="John Doe" />
                    </label>
                    {errors?.name?.type == "required" && <span className=" text-xs font-medium text-red-500">Name is required</span>}
                </div>
            ...
            <p className="mt-8 text-base">Already have an account? <Link to={"/login"} className="text-green-600 hover:underline">Login</Link></p>
